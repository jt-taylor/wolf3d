...
/*   Updated: 2019/10/14 18:05:55 by jtaylor          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static inline void		move_wolf_camera_plane(t_wolf *w, int opt)
{
	...
	{
		old = w->player.dir_x;
		w->player.dir_x = w->player.dir_x *
			cos(-rot) - w->player.dir_y * sin(-rot);
		w->player.dir_y = old * sin(-rot) + w->player.dir_y * cos(-rot);
		old = w->player.plane_x;
		w->player.plane_x = w->player.plane_x *
			cos(-rot) - w->player.plane_y * sin(-rot);
...
static inline void		move_forward_backward(t_wolf *w, int opt)
{
	...
	if (opt == 1)
	{
		(w->map->map[(int)w->player.y_cord]
			[(int)(w->player.x_cord + w->player.dir_x * ms)] == 0) ?
			w->player.x_cord += w->player.dir_x * ms : 0;
		(w->map->map[(int)(w->player.y_cord + w->player.dir_y * ms)]
			[(int)w->player.x_cord] == 0) ?
